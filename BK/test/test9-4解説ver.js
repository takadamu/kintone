      ...
      event.records.forEach(function(record, index){
        relatedRecordsField.getRecords(record).then(function(records){
          if(this.value){
            ...
            // kintone.app.getElementsByClassName("recordlist-row-gaia").style;

            const valrecord = document.getElementsByClassName("recordlist-row-gaia");
            valrecord.classList.add("valrecords");
            document.getElementsByClassName("valrecords");

            // console.log('製番'.value);
            // var obj = document.querySelector(".line-cell-gaia recordlist-ellipsis-gaia");
            // //要素に設定されているclassの一覧を取得する
            // // var list = obj.classList;
            // //要素にclass="hoge"を追加する
            // obj.classList.add("valrecords");
