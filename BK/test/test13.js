    ...
    RelatedRecordsFieldManager.prototype.getFieldProperties().then(function(){
      ...
      event.records.forEach(function(record, index){
        relatedRecordsField.getRecords(record).then(function(records){
          if(records !== null){
            kintone.app.getElementById("recordlist-row-gaia")[index].style.display ="none";
            // document.getElementById("recordlist-row-gaia")[index].style.display ="none";
            // getFieldElements('製番')[index].style.display ="none";
          }
        });
      });
    });
